...
<template>
  ...
  >
    <div class="flex items-start justify-between">
      <div>
        <h3 class="font-bold text-lg text-[var(--text)] group-hover:text-rose-400 transition-colors">{{ cat.name }}</h3>
        <p v-if="cat.breed" class="text-sm text-[var(--text-2)]">{{ cat.breed }}</p>
      </div>
    ...
    <div v-if="cat.weight" class="text-sm text-[var(--text-3)]">
      ⚖️ {{ cat.weight }} kg
    </div>
