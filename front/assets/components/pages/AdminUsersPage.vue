  ...
  <div class="min-h-screen bg-zinc-950 text-zinc-100">
    ...
    <div class="mx-auto max-w-6xl px-6 py-8">
      <div class="mb-6">
        ...
        <p class="text-sm text-zinc-400 mt-1">All registered users · {{ users?.total ?? 0 }} total</p>
      </div>
      ...
      <div v-else class="overflow-hidden rounded-xl border border-zinc-700">
        <table class="w-full text-sm">
          <thead class="border-b border-zinc-700 bg-zinc-900">
            <tr>
              <th class="px-4 py-3 text-left text-xs text-zinc-400 font-medium">Email</th>
              <th class="px-4 py-3 text-left text-xs text-zinc-400 font-medium">Username</th>
              <th class="px-4 py-3 text-left text-xs text-zinc-400 font-medium">Role</th>
          ...
          <tbody class="divide-y divide-zinc-800">
            <tr v-for="user in users?.items" :key="user.id" class="bg-zinc-900 hover:bg-zinc-800/50 transition">
              <td class="px-4 py-3 font-mono text-xs text-zinc-300">{{ user.email }}</td>
              <td class="px-4 py-3">
                <template v-if="editingUser?.id === user.id">
                  <div class="flex items-center gap-2">
                    ...
                    >Save</button>
                    <button class="text-xs text-zinc-500 hover:text-zinc-300" @click="cancelEdit">Cancel</button>
                  </div>
                </template>
                <template v-else>
              ...
              <td class="px-4 py-3">
                <span class="rounded border px-1.5 py-0.5 text-xs font-medium" :class="roleColor(user.role)">
                  {{ user.role.replace('ROLE_', '') }}
                </span>
              </td>
              <td class="px-4 py-3 text-xs text-zinc-500">{{ new Date(user.createdAt).toLocaleDateString() }}</td>
      ...
      <!-- Pagination -->
      <div v-if="users && users.totalPages > 1" class="mt-4 flex justify-center gap-3">
        <button
          :disabled="page <= 1"
