...
<template>
  <BaseModal title="My Account" :open="open" @close="emit('close')">
    <div class="flex flex-col gap-5">
      ...
      <div class="flex items-center gap-3 pb-4 border-b border-[var(--border)]">
        ...
        <div class="min-w-0">
          <p v-if="me?.username" class="font-semibold text-[var(--text)] truncate">{{ me.username }}</p>
          <p class="text-sm text-[var(--text-3)] truncate">{{ me?.email }}</p>
        </div>
      </div>
